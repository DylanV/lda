...
//    Pword[2][9] = 0.1;
    ...
    s = std::vector<double>(numTerms, 1.0);
    gamma = std::vector<std::vector<double>>(numDocs, std::vector<double>(numTopics, ALPHA_INIT));
    beta = std::vector<std::vector<std::vector<double>>>
            (numDocs, std::vector<std::vector<double>>
                    (numTerms, std::vector<double>
                            (numTopics, 0)));
...
double expectation_prop::doc_e_step(int d) {
    ...
    for(auto const& word_count : doc.wordCounts){
        ...
        int token_count = word_count.second;
        ...
        for (int k = 0; k < numTopics; ++k) {
            old_posterior[k] = gamma[d][k] - beta[d][type][k];
        ...
        if (!skipWord) {
            ...
            for (int k = 0; k < numTopics; ++k) {
                beta_new[k] = (1.0-step_size)*beta[d][type][k];
            ...
            for (int k = 0; k < numTopics; ++k) {
                gamma_new[k] = old_posterior[k] + (token_count) * (beta_new[k] - beta[d][type][k]);
                if (gamma_new[k] < 0) {
                    make_changes = false;
                    numSkipped++;
                }
            ...
            if (make_changes) {
                ...
                s[type] = Z_w;
                s[type] *= (tgamma(gamma_prime_sum) / prod_gamma_prime);
                s[type] *= (prod_gamma_old / tgamma(old_posterior_total));
            }
        }
    }
    ...
    for(auto const& word_count : doc.wordCounts) {
        likelihood *= pow(s[word_count.first], word_count.second);
    }
    return likelihood;
}
