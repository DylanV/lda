...
void online_var_bayes::save_parameters(std::string file_dir) {
    ...
    //write theta
    fs.open(file_dir+"theta.dat", fstream::out | fstream::trunc);
    write_2d_vector_to_fs(fs, Elogtheta);
    fs.close();
}
...
void online_var_bayes::train(size_t numTopics) {
    ...
    size_t batchStartId, batchEndID = 0;
    int iter = 0;
    ...
    Elogtheta = vector<vector<double>>(numDocs);
    ...
    for(int run=0; run<numRuns; ++run) {
        ...
        while (batchEndID < numDocs - 1) {
            ...
            for (int i = 0; i < numDocsBatch; ++i) {
                ...
                Elogtheta[batchStartId+i] = dirichlet_expectation(gamma[i]);
            }
            ...
            for (int d = batchStartId; d < batchEndID; ++d) {
                ...
                while (!converged) {
                    ...
                    for (const auto &word_pair : doc.wordCounts) {
                        ...
                        double phisum = 0;
                        for (int k = 0; k < numTopics; ++k) {
                            phi[batchIndex][w][k] = exp(Elogtheta[d][k] + Elogbeta[k][word_id]);
                    ...
                    Elogtheta[d] = dirichlet_expectation(gamma[batchIndex]);
                }
